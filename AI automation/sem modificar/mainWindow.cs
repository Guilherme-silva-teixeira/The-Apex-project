...
namespace WpfApp3
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
        file:///C:/Users/shake/AppData/Local/Temp/RarSFX1/X64/Lang/_XX/Custom/Images/Gallery/5.jpg
            InitializeComponent();
...
[
    ...
    */
                    ...
                    using (JsonDocument document = JsonDocument.Parse(aiResponse))
                    {
                        ...
                        MessageBox.Show("Total de ações (elementos od array)" + count);
                        JsonElement root = document.RootElement;

                        for(int i =0; i < count; i++)
                        {
                            JsonElement actionElement = root[i];
                            await ExecuteSingleAction(actionElement);
                            string action = actionElement.GetProperty("action").GetString();
                            string selector = actionElement.GetProperty("selector").GetString();
                            string text = actionElement.TryGetProperty("text", out var textProp) ? textProp.GetString() : null;
                            string url = actionElement.TryGetProperty("url", out var urlProp) ? urlProp.GetString() : null;
                            int time = actionElement.TryGetProperty("time", out var timeProp) ? timeProp.GetInt32() : 0;
                            pageData.AppendText($"\nAção: {action}, Seletor: {selector}, Texto: {text}, URL: {url}, Tempo: {time}");
                        }
        ...
        private async Task ExecuteSingleAction(JsonElement actionElement)
        {
            ...
            switch (action.ToLower())
            {
                ...
                case "navigate":
                    ...
                    await Task.Delay(200); // Pausa após navegação
                    break;
